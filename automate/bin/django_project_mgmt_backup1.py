    ...
    if os.path.exists(settings_file):
        ...
        for i, line in enumerate(content):
            if "INSTALLED_APPS = [" in line:
                ...
                break
        # Check if STATIC settings already exist
        static_settings = f"""
...
"""
        if "STATIC_URL" not in "".join(content):
            content.append(static_settings)

        # finally write the file
        with open(settings_file, "w") as f:
            f.writelines(content)
        ensure_directories_exist(project_root_path)
        print(f">>> Updated settings.py to include '{app_name}' <<<")

def ensure_directories_exist(project_root_path):
    """Ensure static and media directories exist inside the project folder."""
    static_dir = os.path.join(project_root_path, STATIC_ROOT)
    media_dir = os.path.join(project_root_path, MEDIA_ROOT)

    os.makedirs(static_dir, exist_ok=True)
    os.makedirs(media_dir, exist_ok=True)

    print(f"✅ Ensured {STATIC_ROOT}/ and {MEDIA_ROOT}/ directories exist in {project_root_path}.")



def create_app_urls(app_path, app_name, app_location):
    """Create `urls.py` inside app_<appname> and include mod_app.urls."""
    app_urls_file = os.path.join(app_path, "urls.py")
    mod_app_import_path = f"{app_location}.{app_name}.mod_app.urls"
...
# CREATE THE APP URL ENTRY IN THE PROJECT URLS FILE
def create_project_urls(project_path, app_name):
    """Update `urls.py` in the Django project to include the app URLs."""
    app_location = get_correct_app_location(project_path, app_name)
    project_urls_file = os.path.join(project_path, "urls.py")
    app_import_path = f"{app_location}.{app_name}.mod_app.urls_app"
...
def create_django_app(app_name, base_dir, module_name=None):
    ...
    create_app_urls(app_path, app_name, app_location)
